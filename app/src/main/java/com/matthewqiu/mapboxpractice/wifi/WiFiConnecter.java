...
public class WiFiConnecter {
    ...
    public void connect(String ssid, String password, ActionListener listener) {
        ...
        String quotedString = WifiStringUtils.convertToQuotedString(mSsid);

        // ???????? always true
        boolean ssidEquals = bySsidIgnoreCase ? mSsid.equalsIgnoreCase(info.getSSID())
                : mSsid.equals(info.getSSID());
    ...
    private void handleEvent(Context context, Intent intent) {
        ...
        if (WifiManager.SCAN_RESULTS_AVAILABLE_ACTION.equals(action) && isActiveScan) {
            ...
            for (ScanResult result : results) {
                ...
                String quotedString = WifiStringUtils.convertToQuotedString(mSsid);
                Log.d(TAG,"wifi network found: " + result.SSID);
                boolean ssidEquals = bySsidIgnoreCase ? mSsid.equalsIgnoreCase(result.SSID)
                        : mSsid.equals(result.SSID);
                if (ssidEquals) {
                    //TODO ?
                    Log.d(TAG,"ssid matched");
                    mScanner.pause();
                    //2.input error password
                    if (!WiFi.connectToNewNetwork(mWifiManager, result, mPassword)) {
                        if (mListener != null) {
                            Log.d(TAG,"connect to network failed");
                            mListener.onFailure();
            ...
            Log.d(TAG,"wifi info: " + mWifiInfo.toString());
            Log.d(TAG,"current wifi ssid" + mWifiInfo.getSSID());
            Log.d(TAG,"target wifi ssid" + mSsid);
        ...
        public void handleMessage(Message message) {
            if(message.what==0) {
                if (mRetry < MAX_TRY_COUNT) {
                    ...
                    isActiveScan = false;
                    if (mListener != null) {
                        Log.d(TAG, "scan failed max time");
                        mListener.onFailure();
                        mListener.onFinished(false);
                    }
                    onPause();
    ...
    public void setWiFiConnecterListener(ActionListener listener){
        this.mListener = listener;
    }
}
